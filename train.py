        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                # if global_step % n_train == 0:
                    ...
                    writer.add_images('images', imgs[0:1,0:1,:,:,16], global_step)
                    writer.add_images('images', torch.softmax(masks_pred), global_step)
...
if __name__ == '__main__':
    ...
    net = UNet(n_channels=1, n_classes=8, trilinear=True)
    logging.info(f'Network:\n'
                 f'\t{net.n_channels} input channels\n'
                 f'\t{net.n_classes} output channels (classes)\n'
                 f'\t{"Trilinear" if net.trilinear else "Transposed conv"} upscaling')
